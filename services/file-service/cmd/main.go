...
import (
	...
	"github.com/yourorg/monorepo/services/file-service/internal/config"
	...
	"github.com/yourorg/monorepo/services/file-service/internal/service"
	"go.uber.org/zap"
...
func main() {
	...
	s3Client, err := s3.New(s3.Config{
		...
		SignedURLExpiry: cfg.S3.SignedURLExpiry,
	})
	if err != nil {
	...
	fileService := service.NewFileService(repo, s3Client, logger.Logger)
	...
	// Register services
	filev1.RegisterFileServiceServer(grpcServer, fileService)
	...
	if cfg.HTTP.Enabled {
		...
		err := filev1.RegisterFileServiceHandlerFromEndpoint(ctx, mux, fmt.Sprintf("localhost:%d", cfg.GRPC.Port), opts)
		if err != nil {
		...
		if appMetrics != nil {
			...
			registry.RegisterAll(filev1.GatewayPatterns())
